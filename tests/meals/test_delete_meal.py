...
# Tests
def test_delete_meal_success(client, default_user):
    """Testa a exclusão de uma refeição existente com sucesso"""
    with client:
        # Criação da refeição
        ...
        get_meal = client.get(f"/meal/{meal_id}")
        assert get_meal.status_code == 404
        
def test_delete_meal_unauthorized(client):
    """Testa a exclusão de uma refeição sem login"""
    with client:
        response = client.delete("/meal/1")
        assert response.status_code == 401
        assert response.json['error'] == "Unauthorized access"

def test_delete_meal_user_not_authorized(client):
    """Testa a exclusão de uma refeição com usuário não autorizado"""
    with client:
        ...
        # User2 tenta excluir a refeição de user1
...
def test_delete_meal_not_found(client):
    """Testa a exclusão de uma refeição que não existe"""
    with client:
        create_user(client, 'testuser', 'testpassword')
        login_user(client, 'testuser', 'testpassword')

        response = client.delete("/meal/999")
